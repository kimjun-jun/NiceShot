...
public:
	...
	TransForm					Transform[OBJECT_PLAYER_MAX];			//!< トランスフォーム情報
};
...
public:
	PLAYER_PARAMETER() {
		FrontRotTOaxis = D3DXVECTOR3(0.0f, 0.0f, 0.0f); Bpos = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
		for (int i = 0; i < 3; i++) Bmove[i] = D3DXVECTOR3(0.0f, 0.0f, 0.0f); Brot = 0.0f;
		speed = 0.0f; speedbuff = 0.0f; speedbufftime = 0.0f; MorphingTime = 0.0f;
		BackCameraItemTime = 0.0f; KiriItemTime = 0.0f; AmmoBornTime = 0.0f;
		time = 0.0f; MorphingSignal = 0; vital = PLAYER_VITAL_MAX; oldvital = 0; AmmoStock = 0;
		ModelType = 0; OldModelType = 0; shadowIdx = 0; BFlag = 0;
		GetMorphing = false; Morphing = false; MorphingEnd = false; speedbuffsignal = false;
		BackCameraItemSignal = false; KiriSignal = false; dashFlag = false;
	};
	~PLAYER_PARAMETER() {}
...
*/
class PLAYER_HONTAI : public GAME_OBJECT
{
public:
	PLAYER_HONTAI();	//!< データ読み込み 
	~PLAYER_HONTAI();	//!< 削除
	...
	TransForm			Transform[OBJECT_PLAYER_MAX];			//!< トランスフォーム情報
	PLAYER_PARAMETER	PlayerPara[OBJECT_PLAYER_MAX];			//!< インスタンスに必要なデータ群								
