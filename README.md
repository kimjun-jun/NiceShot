...
C言語で制作したのち、ver2.0からC++を使用しています。現在はC++に書き換え中。

ゲーム説明：
四人対戦戦車TPSゲーム。画面分割にてユーザー4人の対戦を実現。フィールドに落ちてるアイテムを駆使して有利に戦おう。(オンライン化を検討中ー＞本作でオンライン化はしない。代わりに2DゲームのスコアをDB管理、実装予定(FireBaseを使う予定)/2020/09/16)ー＞(「お知らせ」本作でWinsock2を利用したTCP通信対戦をたぶん実装します。/2020/09/22)
